...
bool	separate_block_top_a(t_heads *heads, int block_size, t_lifo **protocol);
...
bool	sort_block_top_a(t_heads *heads, t_lifo **protocol)
{
	...
	else if (!separate_block_top_a(heads, (*protocol)->block_size, protocol))
...
bool	separate_block_top_a(t_heads *heads, int block_size, t_lifo **protocol)
{
	...
	if (!add_amounts_to_protocol_top_a(protocol, block_size))
		return (false);
	return (true);
}
